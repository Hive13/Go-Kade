...
pkgdatadir = $(datadir)/kade
pkglibdir = $(libdir)/kade
pkgincludedir = $(includedir)/kade
...
CONFIG_CLEAN_FILES =  kade.lsm kade.spec
...
CPPFLAGS = 
LDFLAGS = 
LIBS = -lz -lSDL_mixer -lSDL_ttf -lSDL_image -lmysqlclient  -L/usr/lib -lSDL
kade_OBJECTS =  kade.o xmalloc.o getopt.o getopt1.o mame.o menu.o \
SDL_rotozoom.o confile.o sound.o tarball.o sql.o
kade_LDADD = $(LDADD)
kade_DEPENDENCIES = 
kade_LDFLAGS = 
...
INFO_DEPS = kade.info
DVIS = kade.dvi
TEXINFOS = kade.texinfo
man6dir = $(mandir)/man6
man7dir = $(mandir)/man7
MANS = $(man_MANS)

NROFF = nroff
DIST_COMMON =  README $(kade_TEXINFOS) ./stamp-h.in AUTHORS COPYING \
ChangeLog INSTALL Makefile.am Makefile.in NEWS TODO acinclude.m4 \
aclocal.m4 config.h.in configure configure.in kade.lsm.in kade.spec.in
...
TAR = tar
GZIP_ENV = --best
DEP_FILES =  .deps/SDL_rotozoom.P .deps/confile.P .deps/getopt.P \
.deps/getopt1.P .deps/kade.P .deps/mame.P .deps/menu.P .deps/sound.P \
.deps/sql.P .deps/tarball.P .deps/xmalloc.P
SOURCES = $(kade_SOURCES)
OBJECTS = $(kade_OBJECTS)
...
kade.lsm: $(top_builddir)/config.status kade.lsm.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
kade.spec: $(top_builddir)/config.status kade.spec.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
...
kade: $(kade_OBJECTS) $(kade_DEPENDENCIES)
	@rm -f kade
	$(LINK) $(kade_LDFLAGS) $(kade_OBJECTS) $(kade_LDADD) $(LIBS)

kade.info: kade.texinfo $(kade_TEXINFOS)
kade.dvi: kade.texinfo $(kade_TEXINFOS)
...
mostlyclean-aminfo:
	-rm -f kade.aux kade.cp kade.cps kade.dvi kade.fn kade.fns kade.ky \
	  kade.kys kade.ps kade.log kade.pg kade.toc kade.tp kade.tps \
	  kade.vr kade.vrs kade.op kade.tr kade.cv kade.cn
...
install-man: $(MANS)
	@$(NORMAL_INSTALL)
	$(MAKE) $(AM_MAKEFLAGS) install-man6 install-man7
uninstall-man:
	@$(NORMAL_UNINSTALL)
	$(MAKE) $(AM_MAKEFLAGS) uninstall-man6 uninstall-man7
...
installdirs:
	$(mkinstalldirs)  $(DESTDIR)$(bindir) $(DESTDIR)$(infodir) \
		$(DESTDIR)$(mandir)/man6 $(DESTDIR)$(mandir)/man7
...
maintainer-clean-aminfo install-man6 uninstall-man6 install-man7 \
uninstall-man7 install-man uninstall-man tags mostlyclean-tags \
...
install: kade
	...
	mkdir -p $(datadir)/doc/kade/scripts
	cp -f scripts/* $(datadir)/doc/kade/scripts/
